    ...
    def load_flux(self) -> FluxPipeline:
        ...
        try:
            ...
            if config.lora_flux_path:
                ...
                self.flux_pipe.load_lora_weights(config.lora_flux_path)
...
class ImageGenerationTool(BaseTool):
    ...
    def _run(self, prompt: str, negative_prompt: str = "", seed: Optional[int] = None) -> str:
        ...
        try:
            ...
            enhanced_negative = f"low quality, blurry, distorted, amateur, ugly, deformed, {negative_prompt}"
...
Keep the prompt under 100 words but make it rich in visual details. Return ONLY the enhanced prompt, nothing else."""
...
class InteriorDesignAgentSystem:
    ...
    def create_design_analyst_agent(self) -> Agent:
        """Agent for analyzing and enhancing design requests"""
        return Agent(
            role='Senior Interior Design Consultant',
            goal='Transform user ideas into detailed, actionable design prompts',
            backstory="""You are a renowned interior designer with 20 years of experience.
            You excel at understanding client needs and translating them into vivid,
            detailed design concepts that AI can visualize perfectly.""",
            tools=[self.tools["enhance"]],
            llm=qwen_llm,
            max_iter=3,
            verbose=True
        )
    
    def create_image_specialist_agent(self) -> Agent:
        """Agent for generating images"""
        return Agent(
            role='AI Visualization Specialist',
            goal='Generate stunning interior design visualizations from prompts',
            backstory="""You are an expert in AI image generation, specializing in
            architectural and interior visualization. You know exactly how to use
            FLUX.1-dev to create photorealistic interior designs.""",
            tools=[self.tools["generate"]],
            llm=qwen_llm,
            max_iter=3,
            verbose=True
        )
    
    def create_feedback_analyst_agent(self) -> Agent:
        """Agent for analyzing feedback"""
        return Agent(
            role='Client Experience Specialist',
            goal='Analyze feedback and improve designs based on client preferences',
            backstory="""You excel at understanding client feedback and translating
            it into actionable design improvements. You can analyze both the generated
            images and client comments to create better prompts.""",
            tools=[self.tools["feedback"]],
            llm=qwen_llm,
            max_iter=3,
            verbose=True
        )
    
    def create_quality_manager_agent(self) -> Agent:
        """Agent for quality control and saving"""
        return Agent(
            role='Design Quality Manager',
            goal='Ensure design quality and manage the final design collection',
            backstory="""You maintain the highest standards for the design portfolio,
            ensuring only the best designs are saved with proper documentation.""",
    ...
    def save_final_design(self, design_name: str, tags: List[str] = []) -> Dict:
        ...
        try:
            save_task = Task(
                description=f"""Save the final approved design.
                
                Image path: {self.session_data['current_image']}
                Design name: {design_name}
                Tags: {', '.join(tags) if tags else 'interior-design'}
                
                Use the save_final_image tool to save this design.""",
                agent=self.create_quality_manager_agent(),
                expected_output="Save confirmation in JSON format"
            )
            
            # Create and run crew
            crew = Crew(
                agents=[self.create_quality_manager_agent()],
                tasks=[save_task],
            ...
            # Parse result
            parsed_result = self.parse_tool_output(str(result))
            
            if parsed_result.get("success"):
                return {
                    "success": True,
                    "final_path": parsed_result.get("final_path"),
                    "message": f"Design '{design_name}' saved successfully!"
                }
            else:
                return {
                    "success": False,
                    "error": parsed_result.get("error", "Failed to save design")
                }
    ...
    with gr.Blocks(title="Interior Design AI System", theme=gr.themes.Soft()) as interface:
        ...
        **Workflow:**
        ...
        def generate_design(prompt, state):
            ...
            status = "🔄 Generating your design... This may take a minute..."
            yield None, "", 0, status, {}
        ...
        def apply_feedback(feedback, current_image, state):
            if not feedback:
                return current_image, "", 0, "❌ Please provide feedback", {}, state

            if not current_image:
                return None, "", 0, "❌ Please generate an initial design first", {}, state

            status = "🔄 Applying your feedback and generating improved design..."
            yield current_image, "", 0, status, {}, state

            result = system.process_feedback(feedback)
        ...
        def save_design(name, tags, current_image, state):
            if not name:
            ...
            result = system.save_final_design(name, tag_list)
        ...
        def clear_all():
            nonlocal system
            system = InteriorDesignAgentSystem()
            return None, "", 0, "🗑️ Cleared", {}, "", ""
        
        # Connect events
        generate_btn.click(
            fn=generate_design,
            inputs=[prompt_input, state],
            outputs=[output_image, prompt_display, iteration_display, status_output, history_display, state]
        )
        
        iterate_btn.click(
            fn=apply_feedback,
            inputs=[feedback_input, output_image, state],
            outputs=[output_image, prompt_display, iteration_display, status_output, history_display, state]
        )

        save_btn.click(
            fn=save_design,
            inputs=[design_name, tags_input, output_image, state],
        ...
        clear_btn.click(
            fn=clear_all,
            inputs=[],
            outputs=[output_image, prompt_display, iteration_display, status_output, history_display, feedback_input, prompt_input]
        )
